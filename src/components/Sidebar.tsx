...
const Sidebar: React.FC = () => {
  ...
  const updateRule = (polyId: string, idx: number, field: string, value: any) => {
    setPolygons(polygons.map(p =>
      p.id === polyId
        ? {
            ...p,
            colorRule: p.colorRule.map((r: any, i: number) =>
  ...
  return (
    <aside className="w-72 p-6 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-950 border-r border-gray-800 h-full overflow-y-auto shadow-2xl backdrop-blur-2xl flex flex-col gap-6 text-white">
      <h2 className="text-2xl font-extrabold text-cyan-300 mb-2 tracking-tight flex items-center gap-2 neon-glow">
        <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" className="text-cyan-400"><circle cx="12" cy="12" r="10" /></svg>
        Data & Color Rules
      </h2>
      {polygons.length === 0 && <div className="text-gray-500 italic">No polygons yet. Draw one on the map!</div>}
      {polygons.map((poly) => (
        <div key={poly.id} className="mb-6 border-b border-cyan-900 pb-4 bg-gray-900/80 rounded-xl shadow-xl p-3 relative group">
          <button
          ...
          <input
            ...
            value={poly.name}
            onChange={e => setPolygons(polygons.map(p => p.id === poly.id ? { ...p, name: e.target.value } : p))}
            placeholder="Polygon name"
          />
          <div className="mb-2 text-xs text-cyan-400">Data Source: {poly.dataSource}</div>
          <div>
            <div className="font-semibold text-xs mb-1 text-cyan-300">Color Rules</div>
            {poly.colorRule.map((rule: any, idx: number) => (
              <div key={idx} className="flex items-center gap-1 mb-1">
                <select
                  value={rule.op}
                  onChange={e => updateRule(poly.id, idx, 'op', e.target.value)}
                ...
                <input
                  type="number"
                  value={rule.value}
                  onChange={e => updateRule(poly.id, idx, 'value', Number(e.target.value))}
                  className="border border-cyan-700 rounded px-1 py-0.5 w-14 text-xs bg-gray-800 text-cyan-200 focus:border-cyan-400"
                />
                <input
                  type="text"
                  value={rule.color}
                  onChange={e => updateRule(poly.id, idx, 'color', e.target.value)}
                  className="border border-cyan-700 rounded px-1 py-0.5 w-14 text-xs bg-gray-800 text-cyan-200 focus:border-cyan-400"
                />
              </div>
            ))}
